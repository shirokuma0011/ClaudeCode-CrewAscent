...
(function () {
  ...
  /* 目次。第3要素 true = 自分で診断したときだけ出す（共有リンクには回答が無い）。
     節の見出し番号は描画順に振るので、ここに番号は持たせない。 */
  var SECTION_INDEX = [
    ['rs-map', '今回の位置と回答傾向'],
    ['rs-why', 'この位置になった理由', true],
    ['rs-strength', 'この方針が活きる場面'],
    ['rs-firstview', 'サイト設計のたたき台'],
    ['rs-tone', '文章とデザインの方向'],
    ['rs-seo', '検索・運用・業種の考え方'],
    ['rs-near', '近いタイプと相談準備'],
    ['rs-plans', 'プラン比較と相談'],
  ...
  /* 節の番号は描画順に振る。出さない節があっても番号が飛ばない。 */
  var secNo = 0;
  function nextNo() { secNo += 1; return (secNo < 10 ? '0' : '') + secNo; }
  ...
  function render() {
    ...
    var tocList = el('ol');
    SECTION_INDEX.filter(function (it) { return !it[2] || isPersonal; }).forEach(function (it) {
      var li = el('li');
    ...
    m.sec.id = 'rs-map';
    m.box.appendChild(idxHead(nextNo(), 'マップで今回の方向を確認する', '回答時点で、四つの視点のどこを優先したかを示します。'));
    ...
    if (isPersonal && personal.categoryPercents) {
      ...
      t.sec.id = 'rs-trend';
      t.box.appendChild(idxHead(nextNo(), '四つの回答傾向', '回答尺度を0〜100に置き換えた値です。能力、順位、市場適合率、成功率を表す割合ではありません。'));
    ...
    /* ---------- この位置になった理由（personalのみ） ---------- */
    if (isPersonal && personal.axes) {
      root.appendChild(buildWhy(personal.axes, personal.mode));
    }

    /* ---------- 強み / 意識したいバランス ---------- */
    var s = section('band band--sm band--paper');
    s.sec.id = 'rs-strength';
    s.box.appendChild(idxHead(nextNo(), 'この方針が活きる場面と、偏りを防ぐ視点'));
    ...
    if (D.targetVisitor && D.firstViewPlan) {
      ...
      fv.box.appendChild(idxHead(nextNo(), 'ファーストビューの設計', '最初の一画面で何を見せるかの方針です。'));
    ...
    if (D.contentPriority && D.conversionFlow) {
      ...
      pr.box.appendChild(idxHead(nextNo(), '情報の優先順と、問い合わせまでの経路', '上から順に置くと、読み手が判断しやすくなります。'));
    ...
    if (D.toneGuide) {
      ...
      tg.sec.id = 'rs-tone';
      tg.box.appendChild(idxHead(nextNo(), '文章の調子', '同じ内容でも、書き方で伝わり方が変わります。'));
    ...
    if (D.visualRecipe) {
      ...
      vr.sec.id = 'rs-visual';
      vr.box.appendChild(idxHead(nextNo(), '配色、文字、画像、動き', 'デザインを依頼するときに、そのまま渡せる指針です。'));
    ...
    p.sec.id = 'rs-build';
    p.box.appendChild(idxHead(nextNo(), 'サイトの組み立てと問い合わせ導線', '向いているページ構成と機能、デザインの方向です。'));
    ...
    if (D.seoApproach) {
      ...
      seo.sec.id = 'rs-seo';
      seo.box.appendChild(idxHead(nextNo(), '基本SEOの考え方', '順位の予測ではありません。検索する人が何を知りたいかの方向です。'));
    ...
    if (D.avoidPatterns && D.operationTips) {
      ...
      av.sec.id = 'rs-avoid';
      av.box.appendChild(idxHead(nextNo(), '避けたい構成と、公開後にすること'));
    ...
    if (D.industryReasons) {
      ...
      ir.box.appendChild(idxHead(nextNo(), '合いやすい業種と、その理由', '業種そのものではなく、Webサイト上の課題との相性で見ています。'));
    ...
    nz.sec.id = 'rs-near';
    nz.box.appendChild(idxHead(nextNo(), '考え方が近いタイプ', '次点や精度ではありません。方向性が近いタイプなので、比較の参考にできます。'));
    ...
    if (D.consultationQuestions && D.starterCopy) {
      ...
      cq.box.appendChild(idxHead(nextNo(), '制作相談で確認する質問', 'そのまま持ち込めます。答えが決まっていなくても構いません。'));
    ...
    c2.sec.id = 'rs-plans';
    c2.box.appendChild(idxHead(nextNo(), '制作プランを比べる'));
  ...
  /* 回答の5段階を、そのまま画面の言葉に戻す（診断画面と同じ語）。
     render() はこのファイルの上のほうで走るので、var ではなく関数で持つ。 */
  function answerLabel(v) {
    if (v === 2) return 'とても近い';
    if (v === 1) return 'やや近い';
    if (v === 0) return 'どちらともいえない';
    if (v === -1) return 'あまり近くない';
    if (v === -2) return 'まったく近くない';
    return '';
  }

  /**
   * 「この位置になった理由」。
   * 判定は変えない。回答をそのまま観点ごとに足し、どちらへどれだけ寄ったかを見せるだけ。
   */
  function buildWhy(axes, mode) {
    var w = section('band band--sm band--surface');
    w.sec.id = 'rs-why';
    w.box.appendChild(idxHead(nextNo(), 'この位置になった理由',
      '回答をそのまま合計した内訳です。位置の決め方（計算式）は変えていません。'));

    var wrap = el('div', 'why');
    wrap.appendChild(buildWhyAxis(axes.x, false));
    wrap.appendChild(buildWhyAxis(axes.y, true));
    w.box.appendChild(wrap);

    var foot = el('p', 'caption why__foot');
    foot.textContent = '回答済み ' + axes.answered + ' / ' + axes.total + '問' +
      (mode === 'quick' ? '（かんたん診断16問。じっくり診断48問と同じ尺度に置き換えて位置を出しています）' : '（じっくり診断48問）') +
      '。ここに出るのは回答の合計であり、能力・適性・成果を測った値ではありません。';
    w.box.appendChild(foot);
    return w.sec;
  }

  function buildWhyAxis(ax, isVertical) {
    var box = el('div', 'why__ax');
    var max = (ax.left.count + ax.right.count) * 2 || 1;   // 差が取り得る最大値
    var pct = Math.round((Math.abs(ax.diff) / max) * 100);

    box.appendChild(el('h3', 'why__name', ax.name + ' — ' + ax.left.label + ' ⇄ ' + ax.right.label));

    var lean = ax.leaning === 'even'
      ? '左右が同点でした。'
      : (ax.leaning === 'right' ? ax.right.label : ax.left.label) + 'へ ' + Math.abs(ax.diff) + ' 寄りました。';
    box.appendChild(el('p', 'why__lean', lean + '（左右に取り得る幅は ' + max + '）'));

    /* 中央から差の向きへ伸びる帯。値は文でも書いてあるので、色だけに頼らない */
    var bar = el('div', 'why__bar');
    bar.setAttribute('role', 'img');
    bar.setAttribute('aria-label', ax.name + 'は' + lean);
    var fill = el('i', 'why__fill' + (ax.diff < 0 ? ' is-left' : ''));
    fill.style.width = (pct / 2) + '%';
    bar.appendChild(fill);
    box.appendChild(bar);
    var ends = el('div', 'why__ends');
    ends.appendChild(el('span', null, '← ' + ax.left.label));
    ends.appendChild(el('span', null, ax.right.label + ' →'));
    box.appendChild(ends);

    /* マップは縦軸を上下で描く。ここでは帯を横に倒しているので、その対応を書く。 */
    box.appendChild(el('p', 'caption why__note',
      ax.note + (isVertical ? 'この帯は横に倒しています。左が下、右が上にあたります。' : '')));

    var dl = el('dl', 'why__nums');
    [ax.left, ax.right].forEach(function (side) {
      dl.appendChild(coordCell(side.label + 'の合計',
        (side.sum > 0 ? '+' : '') + side.sum + '（' + side.count + '問／どちらともいえない ' + side.neutral + '問）'));
    });
    dl.appendChild(coordCell('どちらともいえない', ax.neutralRate + '%'));
    box.appendChild(dl);

    var tops = ax.left.top.concat(ax.right.top);
    if (tops.length) {
      var t = el('div', 'why__top');
      t.appendChild(el('p', 'why__top-h', 'この軸で特に効いた回答'));
      var ul = el('ul');
      [[ax.left, ax.right], [ax.right, ax.left]].forEach(function (pair) {
        var side = pair[0];
        var other = pair[1];
        side.top.forEach(function (it) {
          /* 近い(+)ならその観点へ、近くない(-)なら反対の観点へ寄る。
             どちらへ効いたのかを、値の符号ではなく言葉で書く。 */
          var toward = it.value > 0 ? side.label : other.label;
          /* 帯と同じ色分け。どちらの極へ効いたかを、線の色でも示す */
          var li = el('li', toward === ax.right.label ? 'is-right' : 'is-left');
          li.appendChild(el('span', 'why__cat', side.label + 'の設問'));
          li.appendChild(el('span', 'why__st', '「' + it.statement + '」'));
          li.appendChild(el('span', 'why__ans',
            '回答: ' + answerLabel(it.value) + ' → ' + toward + 'へ ' + Math.abs(it.value)));
          ul.appendChild(li);
        });
      });
      t.appendChild(ul);
      ...
      box.appendChild(el('p', 'note', 'この軸では、どちらともいえない以外の回答がありませんでした。'));
    }
    return box;
  }
  ...
  function renderUndetermined(personal) {
    document.title = '診断結果: 方向を読み取れませんでした | クルーアセント';
    root.innerHTML = '';
    ...
    c.appendChild(el('h1', 'rs-none__h', '今回の回答からは、方向を読み取れませんでした'));
    ...
    [['もう一度、選び直す', '「良いと思うか」ではなく、Webサイトで今どちらを先にしたいかで選ぶと差が出ます。'],
     ['48問版で答える', '設問が増えると、16問では表れなかった差が出ることがあります。']
    ].forEach(function (r) {
      var li = el('li');
  ...
  function renderInvalid() {
    ...
    c.appendChild(el('p', 'rs-none__note',
      '16問なら2〜3分、48問なら6〜9分です。登録もログインも必要ありません。'));

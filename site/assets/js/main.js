...
(function () {
  ...
  /* ------------------------------------------------------------------
     ...
     ・見出しの一覧から飛んだ先が閉じていたら開く
     ・「すべて開く／すべて閉じる」
     JSが無くても details は自分で開けるので、これは上乗せの機能。
  ------------------------------------------------------------------ */
  function openAncestorDetails(hash) {
    ...
    var d = target.querySelector && target.querySelector('details');
    if (d) d.open = true;
  }
  openAncestorDetails(location.hash);
  window.addEventListener('hashchange', function () { openAncestorDetails(location.hash); });

  Array.prototype.forEach.call(document.querySelectorAll('[data-fold-all]'), function (btn) {
    var scope = document;
    btn.addEventListener('click', function () {
      var all = Array.prototype.slice.call(scope.querySelectorAll('details.fold'));
      if (!all.length) return;
      var anyClosed = all.some(function (d) { return !d.open; });
      all.forEach(function (d) { d.open = anyClosed; });
      btn.textContent = anyClosed ? 'すべて閉じる' : 'すべて開く';
    });
  });
  ...
  /* ------------------------------------------------------------------
     ...
     回答も結果もこの端末の localStorage にしか無い。サーバーには送っていない。
     これまでは「最初からやり直す」で途中の回答しか消せず、出た結果は
     画面から一切消せなかった。共用の端末では次に使う人に見えてしまう。

     鍵の名前を直接書かず crewAscent で始まる項目を集めるので、
     保存版数が上がっても（v4 → v5 など）そのまま動く。
  ------------------------------------------------------------------ */
  var ERASE_LABEL = [
    ['crewAscentDiagnosis', '途中まで答えた回答'],
    ['crewAscentResult', '診断の結果'],
  ];
  function crewStoredKeys() {
    var out = [];
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (k && k.indexOf('crewAscent') === 0) out.push(k);
      }
    } catch (e) { /* localStorage が使えない環境では何も無い扱い */ }
    return out;
  }
  function labelFor(key) {
    for (var i = 0; i < ERASE_LABEL.length; i++) {
      if (key.indexOf(ERASE_LABEL[i][0]) === 0) return ERASE_LABEL[i][1];
    }
    return key;
  }
  function savedAtOf(key) {
    try {
      ...
      var t = d && (d.savedAt || d.updatedAt || d.startedAt);
      if (!t) return '';
      var dt = new Date(t);
  ...
  Array.prototype.forEach.call(document.querySelectorAll('[data-erase]'), function (box) {
    var list = box.querySelector('[data-erase-list]');
    var run  = box.querySelector('[data-erase-run]');
    var msg  = box.querySelector('[data-erase-msg]');
    var none = box.querySelector('[data-erase-none]');

    function draw(justErased) {
      var keys = crewStoredKeys();
      if (list) {
        list.innerHTML = '';
        keys.forEach(function (k) {
          var li = document.createElement('li');
          var at = savedAtOf(k);
          li.textContent = labelFor(k) + (at ? '（' + at + 'に保存）' : '');
          list.appendChild(li);
        });
      }
      var has = keys.length > 0;
      if (list) list.hidden = !has;
      if (run) { run.hidden = !has; run.disabled = !has; }
      if (none) none.hidden = has;
      if (msg && justErased) msg.textContent = 'この端末から削除しました。';
      else if (msg && !justErased) msg.textContent = '';
      box.hidden = false;
    }
    ...
    if (run) {
      run.addEventListener('click', function () {
        ...
        if (!window.confirm('この端末に保存された診断の記録（' + keys.length + '件）を削除します。元に戻せません。よろしいですか？')) return;
        keys.forEach(function (k) { try { localStorage.removeItem(k); } catch (e) { /* 消せなくても続行 */ } });
        draw(true);
        if (window.crewTrack) window.crewTrack('diagnosis_records_erased', { count: keys.length });
      });
    }
    draw(false);
  });
